...
fun main(args: Array<String>){
    ...
    for(i in 0..10){
        println("step: $i")
        printPuzzle(rightPuzzle(puzzle))
    }
...
fun leftPuzzle(puzzle: Array<Array<Int>>) : Array<Array<Int>>{
    var move = false
    for(x in 0..3){
        for(y in 0..3){
            if(y < 3 &&  puzzle[x][y] > 0){
...
fun rightPuzzle(puzzle: Array<Array<Int>>) : Array<Array<Int>>{
    var move = false
    for(x in 0..3){
        for(y in 3 downTo 0){
            if(x > 0 &&  puzzle[x][y] > 0){
                for(xx in (x-1) downTo 0){
                    if(puzzle[x][y] == puzzle[xx][y]){
                        puzzle[x][y] += puzzle[xx][y]
                        puzzle[xx][y] = 0
                        move = true
                        break
                    } else if(puzzle[xx][y] != 0){
                        break
                    }
                }
            }
    ...
    for(x in 0..3){
        for(y in 3 downTo 0){
            if(x < 4 &&  puzzle[x][y] > 0){
                for(xx in (x+1)..3){
                    if(puzzle[xx][y] != 0 && xx - 1 != x){
                        puzzle[xx - 1][y] = puzzle[x][y]
                        puzzle[x][y] = 0
                        move = true
                        break
                    } else if(puzzle[xx][y] == 0 && xx == 3){
                        puzzle[xx][y] = puzzle[x][y]
                        puzzle[x][y] = 0
                        move = true
                    }
                }
